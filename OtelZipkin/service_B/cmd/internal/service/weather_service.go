...
func (s *WeatherService) GetWeather(ctx context.Context, cep string) (Weather, error) {
	...
	ctx, spanGetCityByCep := tracer.Start(ctx, "getCityByCep")
	city, err := s.getCityByCep(ctx, cep)
	if err != nil {
		return Weather{}, err
	}
	spanGetCityByCep.End()

	ctx, spanGetWeatherByCity := tracer.Start(ctx, "getWeatherByCity")
	wheatherApiResponse, err := s.getWeatherByCity(ctx, city)
	if err != nil {
		return Weather{}, err
	}
	spanGetWeatherByCity.End()
